...
class AirDCPP(object):
    ...
    def search_hub(self, search_instance_id, queryline):
        """
    ...
    def check_download_complete(self, bundle_id, filename, max_wait=300, check_interval=5):
        """
                ...
                if response.status_code == 200:
                    ...
                    if status.get('completed', False):
                        ...
                        if os.path.exists(filepath):
                            ...
                            logger.info(f"[AIRDCPP] Download complete: {filename} ({final_size} bytes)")
                            return True
                        else:
                    ...
                    if total > 0:
                        ...
                        logger.info(f"[AIRDCPP] Download progress: {percentage:.2f}% ({downloaded}/{total} bytes)")
        ...
        logger.warn(f"[AIRDCPP] Download timeout for {filename} after {max_wait} seconds")
        return False
